(function () {
    ...
    function switchYesterdayTodayFilterArea (parentArea) {
        var yesterdayElement = parentArea.children[0];
        var todayElement = parentArea.children[1];

        if (yesterdayElement && 
            yesterdayElement.classList.contains ('text-fourth')) {
            // it's today
            yesterdayElement.classList.remove ('text-fourth');
            if (todayElement)
                todayElement.classList.add ('text-fourth');
            return;
        }

        // it's yesterday
        if (yesterdayElement)
            yesterdayElement.classList.add ('text-fourth');

        if (todayElement) 
            todayElement.classList.remove ('text-fourth');
    }

    function switchCloudySunnyFilterArea (parentArea) {
        var parent = parentArea.children[0].children[0];

        var sunnyIcon = parent.children[0];
        var cloudyIcon = parent.children[1];
        var name = parent.children[2];

        if (weatherNames[1] === name.textContent) {
            sunnyIcon.classList.remove ('hidden');
            cloudyIcon.classList.add ('hidden');
            name.textContent = weatherNames[0];
            return;
        }

        sunnyIcon.classList.add ('hidden');
        cloudyIcon.classList.remove ('hidden');
        name.textContent = weatherNames[1];
    }

    function switchDayNightFilterArea (parentArea, hour) {
        var parent = parentArea.children[1].children[0];

        var nightIcon = parent.children[0];
        var dayIcon = parent.children[1];
        var name = parent.children[2];

        if (hour >= 10 && hour <= 20) {
            nightIcon.classList.add ('hidden');
            dayIcon.classList.remove ('hidden');
            name.textContent = timeDay[0];
            return;
        }

        nightIcon.classList.remove ('hidden');
        dayIcon.classList.add ('hidden');
        name.textContent = timeDay[1];
    }
    ...
    this.CUERVAPlugin.prototype.loop = async function (hrs_per_min) {
        ...
        switchYesterdayTodayFilterArea (this.elements[2].inst);
        switchDayNightFilterArea (this.elements[9].inst, this.status.hour);
        setDateStatsArea (
        ...
        while (true) {
            ...
            for (var i = 0; i < data.length; ++i) {
                ...
                if (isOneHour) {
                    ...
                    switchDayNightFilterArea (
                        this.elements[9].inst, 
                        this.status.hour
                    );
            ...
            if (isLastDay (this.status)) {
                switchYesterdayTodayFilterArea (this.elements[2].inst);
                switchCloudySunnyFilterArea (this.elements[9].inst);

            } else if (isLastHour (this.status)) {
                switchYesterdayTodayFilterArea (this.elements[2].inst);
                switchCloudySunnyFilterArea (this.elements[9].inst);
